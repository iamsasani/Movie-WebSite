...
function Movie() {
  ...
  useEffect(() => {
    ...
    async function checkFavoriteStatus() {
      ...
      try {
        const { data } = await axios.get(
          `${BaseUrlMovie}/movie/${id}/account_states?session_id=${session}`,
        );
  ...
  async function handleRateMovie(newValue) {
    ...
    try {
      await axios.post(
        `${BaseUrlMovie}/movie/${id}/rating?session_id=${session}`,
        { value: newValue },
      );
